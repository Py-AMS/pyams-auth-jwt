...
You have to set several security manager properties to use JWT:
    ...
    ...
    zope.interface.exceptions.Invalid: [Invalid('You must define a private and a public key to use RS* or ES* algorithms'...)]
...
Let's try to use another JWT configuration:
    ...
    >>> jwt_configuration.audience = 'app:app1'
...
Using JWT client keys
---------------------

JWT client keys can be used to validate tokens which have been generated by a client for which we have registered
a public key. These tokens must include several attributes matching the registered key:
 - a key ID (`kid` header)
 - an issuer (`iss` claim)
 - an audience (`aud` claim)
 - an object (`obj` claim) with `access` value.

JWT client keys are stored in a JWT client keys container:

    >>> from pyams_auth_jwt.interfaces import IJWTClientKey, IJWTClientKeyContainer
    >>> container = IJWTClientKeyContainer(jwt_configuration)

The first step is to create and save a new JWT client key:

    >>> from datetime import datetime, timedelta, timezone
    >>> from pyams_utils.factory import create_object

    >>> client_key = create_object(IJWTClientKey,
    ...                            key_id='MyKeyID',
    ...                            label='My key label',
    ...                            issuer='PyAMS',
    ...                            audience='PyAMS_auth_jwt',
    ...                            public_key=jwt_configuration.public_key,
    ...                            algorithm=jwt_configuration.algorithm,
    ...                            activation_date=datetime.now(timezone.utc),
    ...                            expiration_date=datetime.now(timezone.utc) + timedelta(hours=1))

    >>> client_key
    <pyams_auth_jwt.client.JWTClientKey object at 0x...>
    >>> container[client_key.key_id] = client_key

Once registered, JWT client keys can be seen as principals:

    >>> principal = plugin.get_principal('jwt:MyKeyID')
    >>> principal
    <pyams_security.principal.PrincipalInfo object at 0x...>
    >>> principal.id
    'jwt:MyKeyID'
    >>> pprint.pprint(plugin.get_all_principals('jwt:MyKeyID'))
    {'jwt:MyKeyID'}
    >>> pprint.pprint(list(plugin.find_principals('key')))
    [<pyams_security.principal.PrincipalInfo object at 0x...>]
    >>> pprint.pprint([principal.id for principal in plugin.find_principals('key')])
    ['jwt:MyKeyID']


We are now going to create a new token using these minimal settings:

    >>> headers = {'kid': 'MyKeyID'}
    >>> payload = {'iss': 'PyAMS', 'aud': 'PyAMS_auth_jwt', 'sub': 'PyAMS_auth_jwt.tests', 'obj': 'access'}
    >>> token = jwt.encode(payload, jwt_configuration.private_key, client_key.algorithm, headers=headers)

    >>> jwt_request = DummyRequest(authorization=('Bearer', token))
    >>> jwt_request.get_jwt_claims = lambda *args, **kwargs: get_jwt_claims(jwt_request, *args, **kwargs)
    >>> pprint.pprint(verify_jwt_token(jwt_request))
    {'status': 'success'}
    >>> pprint.pprint(jwt_request.get_jwt_claims())
    {'aud': 'PyAMS_auth_jwt', 'iss': 'PyAMS', 'obj': 'access', 'sub': 'jwt:MyKeyID'}

We can assign a specific principal to any JWT client key:

    >>> client_key.principal_id = 'users:user1'

    >>> plugin.get_principal('jwt:MyKeyID').id
    'users:user1'

    >>> container.update_key(client_key, None, client_key.principal_id)
    >>> pprint.pprint(jwt_request.get_jwt_claims())
    {'aud': 'PyAMS_auth_jwt', 'iss': 'PyAMS', 'obj': 'access', 'sub': 'users:user1'}

    >>> client_key.principal_id = 'system:admin'
    >>> container.update_key(client_key, 'users:user1', client_key.principal_id)

If disabled or removed, the key authentication is forbidden:

    >>> client_key.enabled = False
    >>> pprint.pprint(verify_jwt_token(jwt_request))
    {'message': 'Forbidden', 'status': 'error'}

    >>> del container[client_key.key_id]
    >>> pprint.pprint(verify_jwt_token(jwt_request))
    {'message': 'Forbidden', 'status': 'error'}
